...
def create_pre_auth_token(user_id: int) -> str:
    ...
    db.execute_query(
        "INSERT INTO pre_auth_token (user_id, token, expires_at) VALUES (%(uid)s, %(tok)s, %(exp)s)",
        {"uid": user_id, "tok": token, "exp": expires_at}, "none"
    )
    return token


def consume_pre_auth_token(token: str) -> Optional[int]:
    row = db.execute_query(
        "SELECT id, user_id FROM pre_auth_token WHERE token = %(tok)s AND expires_at > NOW()",
        {"tok": token}, "single_row"
    )
    if not row:
        return None
    db.execute_query(
